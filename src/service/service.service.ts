...
export class ServiceService extends GenericService<Service> {
  ...
  async getServicesByQuery(query: Query): Promise<[Service[], number]> {
    console.log(query);
    return await this.genericRepository.createQueryBuilder("service")
  ...
  async sumGrossByQuery(dateQuery: DateDto): Promise<AllTimeEarnedDto> {
    return await this.genericRepository.createQueryBuilder("total").select("SUM(gross) as total").where("date >= :startDate AND date <= :endDate", {
